<template>
    <GuestLayout>
        <div class="max-w-3xl mx-auto px-4 py-8">
            ...
            <!-- Back -->
            <button @click="$inertia.back()" class="flex items-center gap-1.5 text-sm text-gray-500 hover:text-green-600 transition-colors mb-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <!-- Header -->
                <div class="bg-gradient-to-r from-green-600 to-green-500 px-5 py-4 flex items-center justify-between flex-wrap gap-3">
                    <div>
                        <p class="text-white/70 text-xs uppercase tracking-wide font-medium">Order Details</p>
                        <p class="text-white font-semibold text-sm mt-0.5">
                            <i class="fas fa-location-dot mr-1.5 text-white/70"></i>
                            {{ order.address }}
                        </p>
                    </div>
                    <div class="flex items-center gap-2 flex-wrap">
                        <span :class="`text-xs font-semibold px-3 py-1 rounded-full ${getStatusBadge(order.status)}`">
                            <i :class="getStatusIcon(order.status)" class="mr-1 text-[10px]"></i>
                            {{ order.status }}
                        </span>
                        <span :class="`text-xs font-semibold px-3 py-1 rounded-full ${order.paid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`">
                            <i :class="order.paid ? 'fas fa-check' : 'fas fa-clock'" class="mr-1 text-[10px]"></i>
                ...
                <div class="divide-y divide-gray-50">
                    ...
                    >
                        ...
                        <div class="text-right flex-shrink-0">
                            ...
                            <button
                                v-if="order.status === 'Delivered' && !row.rate"
                                @click="addRating(row.id)"
                                class="mt-1.5 flex items-center gap-1 text-xs text-yellow-600 hover:text-yellow-700 font-medium transition-colors"
                            >
                                <i class="fas fa-star text-[10px]"></i> Rate
                            </button>
                            <span v-else-if="order.status === 'Delivered' && row.rate" class="text-xs text-gray-400 flex items-center gap-1 mt-1.5">
                ...
                <!-- Footer summary -->
                <div class="bg-gray-50 border-t border-gray-100 px-5 py-4 flex items-center justify-between flex-wrap gap-3">
        ...
        <div>
            ...
            >
                <div class="flex flex-col items-center py-4">
                    ...
                    <p class="text-sm text-gray-500">Drop files here or <em class="text-green-600 not-italic font-medium">click to upload</em></p>
