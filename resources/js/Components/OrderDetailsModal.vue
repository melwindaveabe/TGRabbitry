<template>
        ...
        <div class="px-4">
            <!-- <el-card class="mt-4" shadow="always"> -->
                <div class="text-lg mb-5">
                    Delivery Address: {{ props.order.address }}
                </div><br>
    
                <el-table
                    :data="props.order.details"
                    row-key="id"
                ...
                <div class="flex justify-between gap-2 mt-5 flex-wrap">
                    <div class="w-full md:w-[49%]">
                        <div>
                            Payment Method: {{ props.order.payment_method }}
                        </div>
                        
                        <div class="mt-3">
                            Payment Status: 
                            <el-tag v-if="props.order.paid" type="success">Paid</el-tag>
                    ...
                    <div class="w-full md:w-[49%] md:text-right">
                        <div>
                            Order Status: 
                            <el-tag :type="getTagType(props.order.status)">{{ props.order.status }}</el-tag>
                        </div>
                        <el-button 
    ...
    <el-dialog 
        ...
        <img loading="lazy" :src="`${$page.props.base_img_path}website/header-logo.png`" class="w-[70%] lg:w-[300px]" alt="">
...
watch(showOrderModal, (val, _) => {
    ...
    function onCancel(){
        axios.post(route('cart.cancel', props.order.id))
            .then(function(){
                ElMessage.success('Order canceled successfully')
                props.onClose(true)
